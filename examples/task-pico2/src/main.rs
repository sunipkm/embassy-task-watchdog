...
use embassy_task_watchdog::embassy_rp::{Watchdog, watchdog_run};
...
async fn watchdog_task(watchdog: &'static Watchdog) -> ! {
...
async fn main_task(watchdog: &'static Watchdog) -> ! {
    loop {
        // Feed the watchdog
        watchdog.feed().await;
        // Do some work
...
async fn second_task(watchdog: &'static Watchdog) -> ! {
    loop {
        // Feed the watchdog
        watchdog.feed().await;
        // Do some work
